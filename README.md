...
### Flush data collection program to the sensor:
...
6. Set the sensor mode to 'flush' mode by setting switch combinations shown in user guide (look for 'user guide' here: https://www.ti.com/tool/IWR6843AOPEVM#order-start-development)
7. Reset the sensor by pressing the reset button
...
10. Change the sensor mode to flash by pressing switch combinations shown in the user guide
...
### General information:
- The mmwave radar sensor needs to flush a binary program which we do using uniflush. This is one time process to install the program.
- The flushed program can be modified by inserting the config parameters using CLI command port
